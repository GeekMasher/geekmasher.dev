...
updated: "2020-08-22T14:00:00.000Z"
template: "post"
draft: false
...
There was a number of things that got me very interested recently about the tool and why I'm writing this blog article.
...
The [Static Application Security Testing (SAST)][sast] product is built out of multiple core components that we'll need to understand their role and purpose.
...
This allows us to take any code base which the engine supports, run the CodeQL engine on that code base and create a CodeQL Database which is the next core component that we'll cover.
...
This generates a large amount of data and facts about your code base that can be used to query and identify items and flows in the code  such as security vulnerabilities.

Both the Engine and Database work very closely together in the form of the CLI (which we'll cover later on in this post).
...
The language is both a Structured Query Language (SQL) and a Object-Oriented Programming (OOP) like programming language.
...
Queries can be very complex using methods to perform specific tasks, importing of other queries for modularity and commonly used code, and condition statements to exclude or include data based of specific requirements.

In this blog post I will not be covering in depth how to write CodeQL Queries but if you want to learn more please check out the follow references:
...
I will also be creating a post on this topic in the near future so keep tuned.

## Initial Setup

<!-- TODO: update as needed -->
Before we start scanning code, we need to setup the correct environment we need to do so.
This involves four core steps; setting up the CLI, Query Packs and an application to scan.

### Setting up CodeQL CLI

<!-- Introduction to section -->
To setup [CodeQL as a command line utility][codeql-cli] you with need todo a few different things.
...
This repository doesn't contain any source code for the CLI but it does have the binaries needed to run CodeQL.
Once downloaded, we need to extract the `codeql` folder into our CodeQL root folder.
I recommend calling the extracted folder something along the lines of "codeql-cli" or "codeql-binaries" to make it easy to recognize it.

<!-- Adding CodeQL to your path -->
You may want to add CodeQL to you shell's path.
...
A Query (QL) Pack is a collection of queries, library files, query suites, and important metadata use for the analysis.
...
Once you have both the CLI installed and the queries for the languages in the CodeQL folder, you'll want to make sure everything is setup correctly by resolving both the current supported languages and query packs.

The first command will list all of the CodeQL extractor packs available on the system.
You should have 7 or more different languages all pointing to folders in the CLI folder.
...
You should have multiple folders pointing to the core language pack along with tests, upgrades, and examples packs.
...
<!-- Query suites -->
Now that we have all the public queries we will need to analysis the code, we can now use an existing or create our own collection of queries we want to run called Query Suites.
If you want to create your own Query Suite, follow [this guide][codeql-query-suites].
We will be using the built in one that are inside the [javascript query pack][codeql-query-suites-js].

### Selecting the Code Base
...
Lets take our project that we will be analysing and run the CLI tool to generate the Database.

Below you can see two `codeql` commands:
The first is to analysis the [Pixi project][pixi] (JavaScript, interpreted).
The second is an example where you're analysing Java (compiled) code from [OWASP WebGoat][webgoat] project. 
...
You can run `codeql database create --help` to get a full list of commands and details of each argument but here is a short summary for each one.

- `./databases/Pixi` *[database path] (required)*

<!-- TODO: Rewrite -->
This is the main section of the command is the `database create` which selects database creation command in the CLI (lots of different commands).
After that the user can select the name of the Database.
...
Now lets take a look at an example output of the command looks like (ran in PowerShell):

![CodeQL Database Create Output](/media/sast/codeql-introduction/CodeQL-DatabaseCreateOutput.PNG)

You will notice in the logs all the files that are being analysed by the CodeQL engine and the resulted placed into the database.
...
Now we have own database (either from generating one or downloading one) we can move on to the fun bit;
running queries.
...
*Note: Sometime you maybe required to update the database before running the `upgrade` command*
...
You can run `codeql database analyze --help` to get a full list of commands and details of each argument but here is a short summary for each one.
...
The format argument tells the CodeQL to what format do you want to export the data generated by the engine to.
The two main formats are `csv` (easy to read) and SARIF, more about that in a minute.

- `--output` *(required)*

<!-- TODO: Redo -->
The output option specifies the location to which the content format will be saved too.  
...
The final options selects the query, query suite or directory you want to use to analysis the results with.
In this example, I'm selecting a single query from the [CodeQL core query pack][codeql-queries].

Note that typically to scan multiple queries, you use a Query Suite.
An example of this would be using the follow:

```
./queries/javascript/ql/src/codeql-suites/javascript-code-scanning.qls
```

#### Analysis Output

<!-- TODO: redo text below -->
After running for a couple of seconds to a minute, you should have some results like the example output below (ran in PowerShell).
...
Thankfully because we have have run this command once, when you run it again it's almost instance. 
 
<!-- Output Formats -->
Once the query has compiled amd evaluated the code, the resulting file is produced.
In our case a CSV file with a resulting Sinks where the issues occurs.

<!-- TODO: CVS screenshot -->

This is a great format just to see quickly some results but it does lack some of the full flow paths typical security researching need to understand the exact path from source to sink;
This is where SARIF comes in.

<!-- TODO: Link to standard -->
The other format I talked about earlier is the SARIF or Static Analysis Results Interchange Format.
This structured JSON data generates full execution paths for users to follow and all the details you could need for verifying the results.
...
<!-- TODO: finalise the database -->

## Conclusion
 
<!-- Conclusion -->
In this blog post we have cover a number of different topics that hopefully you have learnt and taken away to help you know more about CodeQL.
The first sections was primarily focused on understand what CodeQL, how generally it works, and how to setup your local environment to get started.
...
8. [CodeQL CLI Download](https://github.com/github/codeql-cli-binaries/releases)  
9. [Visual Studio Code plugin](https://marketplace.visualstudio.com/items?itemName=GitHub.vscode-codeql)
10. [CodeQL Queries](https://github.com/github/codeql)
11. [CodeQL GoLang Queries](https://github.com/github/codeql-go)
12. [Query Suites](https://help.semmle.com/codeql/codeql-cli/procedures/query-suites.html)
13. [Query Suites - JavaScript](https://github.com/github/codeql/tree/main/javascript/ql/src/codeql-suites)
14. [Creating CodeQL databases](https://help.semmle.com/codeql/codeql-cli/procedures/create-codeql-database.html)
15. [GitHub Satellite 2020 Workshop - CodeQL for Java/JavaScript](https://github.com/githubsatelliteworkshops/codeql)
16. [Starter workspace to use with the CodeQL extension for Visual Studio Code.](https://github.com/github/vscode-codeql-starter)
17. [LGTM Website](https://lgtm.com/)
18. [DevSlop Pixi](https://github.com/DevSlop/Pixi)
19. [Wikipedia - Lexical Analysis Tokenization](https://en.wikipedia.org/wiki/Lexical_analysis#Tokenization)
20. [Wikipedia - Abstract syntax tree](https://en.wikipedia.org/wiki/Abstract_syntax_tree)
21. [Wikipedia - Control-flow graph](https://en.wikipedia.org/wiki/Control-flow_graph)
22. [Wikipedia - Data-flow analysis](https://en.wikipedia.org/wiki/Data-flow_analysis)
23. [Wikipedia - Static application security testing](https://en.wikipedia.org/wiki/Static_application_security_testing)
24. [OWASP WebGoat](https://github.com/webgoat/webgoat)
25. [YouTube - Finding security vulnerabilities in Java with CodeQL - GitHub Satellite 2020](https://www.youtube.com/watch?v=nvCd0Ee4FgE)
26. [YouTube - Finding security vulnerabilities in JavaScript with CodeQL - GitHub Satellite 2020](https://www.youtube.com/watch?v=pYzfGaLTqC0)
...
[github-security]: https://github.com/features/security "GitHub - Securing software, together"
